...
async function getEpisodeServers(url: string): Promise<types.EpisodeServer[]> {
  const servers: types.EpisodeServer[] = [];
...
async function getLastEpisodes(): Promise<types.Episode[]> {
  const episodes: types.Episode[] = [];
  const $ = cheerio.load((await axios.get(PageInfo.url)).data);
  $("ul.row.row-cols-xl-4.row-cols-lg-4.row-cols-md-3.row-cols-2")
    .find("li")
    .each((_i, element) => {
...
async function getAnimeEpisodes(
  ...
  for (let i = 1; i <= length; i++) {
    ...
    episode.url = api.getEpisodeURL(
      PageInfo,
      `https://monoschinos2.com/ver/${animePath}-episodio-${i}`,
    );
...
function getAnimeCalendar(strDate: string): ClimaticCalendar {
  ...
  if (date.length != 5) return { year: 0, station: null };
  else {
    for (let i = 0; i < calendar.length; i++) {
      if (calendar[i][0].includes(date[2].toLowerCase())) {
        return {
          year: parseInt(date.pop()),
          station: calendar[i][1].toString(),
        };
      }
    }
  }
...
async function getLastAnimes(url?: string): Promise<types.AnimeMedia[]> {
  ...
  if (url === null) {
    $("ul.row.row-cols-2.row-cols-sm-3")
      .find("li")
