...
async function getEpisodeByElement($, element): Promise<types.Episode> {
  ...
  episode.image = $(element)
    .find("div.animeimgdiv img.animeimghv")
    .attr("data-src");
...
function getAnimeEpisodes($): types.Episode[] {
  let episodes: types.Episode[] = [];
  $("div.heromain2 div.allanimes div.row")
    .children()
    .each((_i, element) => {
      const episode = new types.Episode();
...
function getAnimeCalendar(element): ClimaticCalendar {
  const date = element
    .find("ol.breadcrumb li.breadcrumb-item")
    .text()
    .trim()
    .split(" ");
...
async function getLastAnimes(url?: string): Promise<types.Anime[]> {
  ...
  for (let i = 0; i < elements.length; i++) {
    ...
    if (
      utils.isUsableValue(href) &&
      href !== "https://monoschinos2.com/emision?p=2"
    ) {
...
export class Monoschinos {
  ...
  async filter(
    ...
    const link = utils.isUsableValue(name)
      ...
      : `${PageInfo.url}/animes?categoria=${category ?? false}&genero=${
          genre ?? false
        }&fecha=${year ?? false}&letra=${letter ?? false}`;
