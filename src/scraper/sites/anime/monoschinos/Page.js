...
async function getAnime(url) {
  ...
  const $ = cheerio.load(
    (await axios.get(`${PageInfo.url}/anime/${url}`)).data
  );
...
async function getLastAnimes(url) {
  try {
    ...
    const $ = cheerio.load(
      (await axios.get(use_filter ? url : `${PageInfo.url}/emision`)).data
    );
...
async function getFilterAnime(category, genre, year, letter) {
  try {
    let animes = [];
    const $ = cheerio.load(
      (
        await axios.get(`${PageInfo.url}/animes`, {
          params: {
