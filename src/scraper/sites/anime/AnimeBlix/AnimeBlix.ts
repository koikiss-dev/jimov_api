...
export class AnimeBlix extends AnimeProviderModel {
  ...
  async GetAnimeInfo(anime: string): Promise<Anime> {
    try {
      ...
      const AcceptAlts = $(".cn .info .r .u")
        .next()
        .find("li")
        .text()
        .replace("Nombre original: ", "")
        .replace("Nombre en inglés: ", "---")
        .replace("Nombre en japones: ", "---");
      ...
      const AnimeInfo: Anime = {
        ...
        status: AnimeStatus,
        date: AnimeDate[0]
          ? { year: DateFormat[2], month: DateFormat[1], day: DateFormat[0] }
          : null,
        episodes: [],
      };
      ...
      const ListEpisode = ListEpisodeIndex.slice(
        ...
        .slice(0, ListEpisodeIndex.lastIndexOf(",", -1))
        .split(",");
