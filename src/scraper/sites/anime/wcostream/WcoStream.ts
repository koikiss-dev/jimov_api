  ...
  async GetItemInfo(anime: string): Promise<AnimeMedia> {
    try {
      const { data } = await axios.get(`${this.url}/anime/${anime}`, {
        headers: {
          "User-Agent":
            "Mozilla/5.0 (Linux; Android 10; LM-K920) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.106 Mobile Safari/537.36",
        },
      });
      ...
      const name = $(
        ...
        .text()
        .replace("Share On", "");
      const genre = $(".ui-grid-solo.left .ui-block-a")
        .text()
        .replace("Genre;", "")
        .replace("Language; ", "");
      ...
      const AnimeInfo: AnimeMedia = {
        ...
        synopsis: $("#category_description .ui-grid-solo .ui-block-a div p")
          ...
          .replace(/\n/g, ""),
        image: {
          url: !image.includes("https://")
            ? image.replace("//", "https://")
            : image,
        },
        genres: [
          ...genre
            .replace(genre.includes("Dubbed") ? "Dubbed" : "Subbed", "")
            .trim()
            .replace(/\n/g, "")
            .replace(/\s+/g, "")
            .replace("-", "")
      ...
      $("ul.ui-listview-z li").map((_i, e) => {
        ...
        const episode = data
          ...
          .trim()
          .replace(/[^0-9-.]/g, "");
        const season = data.includes("Season")
          ? data
              .slice(data.search(" Season "), data.search(" Episode "))
              .replace("Season", "")
              .trim()
          : "";
        ...
        if (data && !data.includes("Movie") && !data.includes("OVA")) {
          const AnimeEpisode: Episode = {
            ...
            num: Number(episode),
            thumbnail: {
              url: `https://cdn.animationexplore.com/thumbs/${$(e)
                .find("a")
                .attr("href")
  ...
  async GetEpisodeServers(episode: string, season: number | string) {
    try {
      ...
      $$("item").each((_i, e) => {
        ...
        if (
          title.includes("Episode " + NumEpisode + " ") &&
          !season &&
          !title.includes("Season")
        ) {
          AnimeEpisodeInfo.name = title
            .replace("<![CDATA[", "")
            .replace("]]>", "")
            .trim();
          AnimeEpisodeInfo.thumbnail.url = $$(e)
            .find("jwplayer[type='image']")
            .text();
          const Server: EpisodeServer = {
            name:
              "JWplayer - " +
              $$(e).find("jwplayer[type='video']").attr("label"),
    ...
    try {
      ...
      const animeSearch: ResultSearch<AnimeResult> = {
        nav: {
          ...
          next:
            $("#blog .cerceve").length < 28 ? 0 : page ? Number(page) + 1 : 2,
