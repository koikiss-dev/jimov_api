    ...
    for (let i = 0; i < table_downloads.length; i++) {
      ...
      if (!(episode_server == undefined || episode_server == null)) {
        ...
        servers.push({
          name: server,
          url: null,
          file_url: $(table_downloads[i]).find("a").attr("href"),
        });
      }
    }
  } catch (error) {
...
function getEpisode($, element) {
  ...
  episode.url = $(element)
    ...
    .attr("href")
    .replace("/ver/", "/anime/tioanime/servers/");
...
function getScriptAnimeInfo($) {
  ...
  try {
    ...
    script =
      script.substring(0, script.indexOf("var episodes_details")) +
...
async function getAnime(url) {
  ...
  anime.date = new types.DatePeriod(
    new types.Calendar(
      data.info.length < 4
...
export class TioAnime {
  ...
  async filter(
    ...
    if (!(usable = utils.isUsableValue(name) && name.trim().length != 0))
      year_range ??
        (year_range = { begin: 1950, end: new Date().getFullYear() });
    (
      await getLastAnimes(
        `${PageInfo.url}/directorio?${
          usable
            ? `q=${name}`
