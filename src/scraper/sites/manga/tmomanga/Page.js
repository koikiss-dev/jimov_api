...
import {
  ...
  ChapterView,
  Manga,
} from "../../../../utils/manga/schemaProviders.js";
...
async function getChapter(url) {
  ...
  chapter.images = $("div#images_chapter img")
    .map((i, el) => {
      if (i === 0) chapter.cover = $(el).attr("data-src");
...
async function getManga(url) {
  ...
  manga.genres = $(data.get(1))
    .find("a")
    .map((i, el) => {
      return $(el).text().trim();
    })
    .toArray();
  manga.chapters = $("ul.sub-chap li")
    .children()
    .map((i, el) => {
      return $(el).attr("href");
    })
...
async function getSectionContent(number) {
  try {
    ...
    const elements = $($("div.main-col-inner").get(number))
      .find("div.row")
      .children();
    for (let i = 0; i < elements.length; i++) {
      content.push(
        number == 1
          ? await getManga($(elements[i]).find("h3 a").attr("href"))
