...
function getSearchURL(name, genre) {
  if (utils.isUsableValue(genre))
    url = `https://tmomanga.com/genero/${genre
      .toLowerCase()
      .replace(" ", "-")}`;
  else if (utils.isUsableValue(name))
    url = `https://tmomanga.com/biblioteca?search=${encodeURI(name)
      .split("%20")
      .join("+")}`;
  return null;
}
...
export async function filter(name, genre, page) {
  try {
    ...
    if (search_url != null) {
      let mangas = [];
      if (utils.isUsableValue(page)) search_url += `?page=${page}`;
      const $ = cheerio.load((await axios.get(search_url)).data);
      const elements = $($("div.main-col-inner").get(0))
        .find("div.row")
        .children();
