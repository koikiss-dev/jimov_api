...
export class Manganelo extends MangaScraperModel {
  ...
  private GetMangaStatus(data: cheerio.Root) {
    ...
    if (selector.text().trim() == "Ongoing") return "ongoing";
  ...
  private GetMangaPages(data: cheerio.Root) {
    ...
    return data("div.container-chapter-reader > img")
      .map((_, element) => data(element).attr("data-src"))
      .get();
  }
  ...
  private GetMangaSearchResults(data: cheerio.Root): IMangaResult[] | null {
    ...
    return section
      .find("div.content-genres-item")
      .map((_, element) => {
        const mangaResultId = data(element)
          .find("a.genres-item-img")
          .attr("href")
          .split("-")
          .at(-1);
  ...
  async GetItemInfo(mangaId: string) {
    ...
    const chapters = $("div.panel-story-chapter-list")
      .find("ul > li.a-h")
      .map((_, element) => {
  ...
  async GetMangaChapters(mangaId: string, chapterNumber: number) {
    ...
    const name =
      $("body > div.body-site > div:nth-child(1) > div.panel-breadcrumb > a")
        .eq(-1)
        .attr("title") || null;
    const chapter = new MangaChapter();
